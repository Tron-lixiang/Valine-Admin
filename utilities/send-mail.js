...
exports.notice = (comment) => {
  ...
  let _template =
      ...
      '<div style="border-top:2px solid #12ADDB;box-shadow:0 1px 3px #AAAAAA;line-height:180%;padding:0 15px 12px;margin:50px auto;font-size:12px;"><h2 style="border-bottom:1px solid #DDD;font-size:14px;font-weight:normal;padding:13px 0 10px 8px;">        您的<a style="text-decoration:none;color: #12ADDB;" href="${SITE_URL}" target="_blank">${SITE_NAME}</a>上有了新的评论</h2><p><strong>${NICK}</strong>回复说：</p><div style="background-color: #f5f5f5;padding: 10px 15px;margin:18px 0;word-wrap:break-word;">            ${COMMENT}</div><p>您可以点击<a style="text-decoration:none; color:#12addb" href="${POST_URL}" target="_blank">查看回复的完整內容</a><br></p></div></div>';
  ...
  if (process.env.SC_KEY != null) {
    const ScTextTemplate = `您的`+process.env.SITE_NAME+`网站上，有新评论啦～`;
    const ScDespTemplate = `「`+comment.get("nick")+`」回复:【`+
        comment.get("comment")+`】`+`\r###点击[查看完整內容]`+`(`+process.env.SITE_URL + comment.get("url")+"#" + comment.get("objectId")+`)###`;

    const _DespTemplate = ScDespTemplate;
    const _TextTemplate = ScTextTemplate;
    request(
        {
        ...
        function (error, response, body) {
          ...
          if (response.statusCode === 200){
            console.log("已发送微信提醒: %s", response);
          }else{
            console.warn("微信提醒失败:", body, response);
          }
        }
    );
  }
